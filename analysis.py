...
class AdaptivePushupAnalyzer:
    ...
    def process_point(self, current_time, shoulder):
        # 2. Track Shoulder
        ...
        if self.state == 'CALIBRATING':
            if self.avg_amp_shoulder > 0:
                calib_thresh_shoulder = 0.30 * self.avg_amp_shoulder
                
                # BOTH requirements must start moving down
                if val_shoulder < self.global_max_shoulder - calib_thresh_shoulder:
                ...
                elif val_shoulder > self.global_min_shoulder + calib_thresh_shoulder:
        ...
        # 4. LOOKING FOR A MAXIMUM (Upward phase)
            ...
            # REQUIREMENT: fulfilled to transition states
            if shoulder_dropped:
                self._register_peak('Max', self.local_extreme_time, self.local_extreme_shoulder)
        ...
        # 5. LOOKING FOR A MINIMUM (Downward phase)
            ...
            # REQUIREMENT: fulfilled to transition states
            if shoulder_risen:
                self._register_peak('Min', self.local_extreme_time, self.local_extreme_shoulder)
    ...
    def _register_peak(self, peak_type, t, val_shoulder):
        self.peaks.append({
            'type': peak_type, 
            'time': t, 
            'shoulder': val_shoulder
        })
        print(f"[{self.name}] {t:>5.2f}s | Confirmed {peak_type:<3} | Shoulder Ex.: {val_shoulder:.2f}")
        ...
        if len(same_type_peaks) >= 2:
            ...
            if peak_type == self.peaks[0]['type']: 
                self.rep_count += 1
                print(f"-> Rep {self.rep_count} completed! Period: {period:.2f}s | Avg Shoulder Range: {self.avg_amp_shoulder:.2f}\n")
...
def stream_csv_data(angles_filepath, shoulder_filepath):
    ...
    for index, row in df.iterrows():
        ...
        analyzer.process_point(t, shoulder)

    print(f"\n--- Final Statistics ---")
    print(f"Total Valid Reps (Shoulder): {analyzer.rep_count}")
